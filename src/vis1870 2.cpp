...
//    message.Printf("	%d Characters with size: 6x%d\n", pcbMask_+1, linesPerCharacters_);
    ...
    switch (vis1870Configuration_.statusBarType)
    {
        ...
        case STATUSBAR_CIDELSA:
            ...
            if (vis1870Configuration_.statusBarLedOut != -1)
            {
                p_Main->message("Configuring Statusbar");
                message.Printf("	Output %d: led on/off\n", vis1870Configuration_.statusBarLedOut);
                p_Main->message(message);
                p_Computer->setOutType(vis1870Configuration_.statusBarLedOut, LED_CIDELSA_OUT);
            }
        break;
    }
...
void VIS1870::cycle1870()
{
    ...
    if (cycleValue_ == nonDisplayPeriodEnd_)
    {
        ...
        if (changeScreenSize_)
        {
            ...
            p_Main->setMainSize(size); // TO BE CHECKED - what size is set here? Currently size isn't used, did I remove too much?
...
{
    if (vis1870Configuration_.statusBarType == STATUSBAR_DIAG)

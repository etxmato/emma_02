...
class NetronicsScreen : public Panel
{
public:
	NetronicsScreen(wxWindow *parent, const wxSize& size, int tilType);
	~NetronicsScreen();
...
class Netronics : public wxFrame, public MainElf
{
public:
	Netronics(const wxString& title, const wxPoint& pos, const wxSize& size, double clock, ElfConfiguration conf);
	Netronics() {};
	~Netronics();
...
private:
	class NetronicsScreen *elf2ScreenPointer;

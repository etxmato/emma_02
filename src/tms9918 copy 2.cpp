...
const wxBitmap spritesClear(320, 240);
...
: Video(title, pos, size)
{
    ...
    dcMemoryMainPlane.SelectObject(*spritePlanePointer);
...
void Tms9918::drawSprites()
{
    ...
    if (mode_ != TMS_TEXT)
        dcMemory.Blit(offsetX_, offsetY_, videoWidth_, videoHeight_, &dcMemoryMainPlane, offsetX_, offsetY_);
...
//    dc.Blit(0, 0, videoWidth_+2*offsetX_, videoHeight_+2*offsetY_, &dcMemoryMainPlane, 0, 0);

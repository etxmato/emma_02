...
//    computerConfiguration.nvRamDisableDefault = false;
    ...
    computerConfiguration.frontPanelConfiguration.flexPanel = false;
    computerConfiguration.frontPanelConfiguration.type = PANEL_NONE;
    computerConfiguration.ledDisplayConfiguration.defined = false;
    computerConfiguration.useSwitch = false;
    computerConfiguration.useHex = false;
    computerConfiguration.frontPanelConfiguration.defined = false;
    ...
    computerConfiguration.frontPanelConfiguration.tilType = TILNONE;
    ...
    computerConfiguration.ledDisplayConfiguration.q = false;
    computerConfiguration.ledDisplayConfiguration.run = false;
    computerConfiguration.ledDisplayConfiguration.load = false;
    computerConfiguration.ledDisplayConfiguration.reset = false;
    computerConfiguration.ledDisplayConfiguration.pause = false;
    computerConfiguration.ledDisplayConfiguration.error = false;
    computerConfiguration.ledDisplayConfiguration.ready = false;
    computerConfiguration.ledDisplayConfiguration.stop = false;
    computerConfiguration.ledDisplayConfiguration.power = false;
    computerConfiguration.ledDisplayConfiguration.showDataOnLoad = false;
    computerConfiguration.frontPanelConfiguration.dataSwitches = false;
    computerConfiguration.frontPanelConfiguration.powerButtonDefined = false;
    computerConfiguration.frontPanelConfiguration.waitButtonDefined = false;
    ...
    for (int bit=0; bit<MAX_BIT_LEDS; bit++)
        computerConfiguration.ledDisplayConfiguration.bit[bit] = false;
    for (int bit=0; bit<MAX_CPU_STATE_LEDS; bit++)
        computerConfiguration.ledDisplayConfiguration.cpuStatus[bit] = false;
    ...
    computerConfiguration.ideFileConfiguration.directory = computerConfiguration.mainDir_ ;
     ...
     //           parseXml_System (*child);
            ...
            case TAG_PANEL:
                ...
                if (child->GetAttribute("init") == "on")
                    computerConfiguration.frontPanelConfiguration.defined = true;
                if (child->HasAttribute("type"))
                {
                    if (child->GetAttribute("type") == "io")
                    {
                        computerConfiguration.frontPanelConfiguration.defined = false;
                        parseXml_FrontPanel (*child);
                    }
                    if (child->GetAttribute("type") == "cosmac")
                    {
                        computerConfiguration.frontPanelConfiguration.tilType = TIL311;
                        computerConfiguration.frontPanelConfiguration.type = PANEL_COSMAC;
                        parseXml_FrontPanel (*child);
                    }
                    if (child->GetAttribute("type") == "netronics")
                    {
                        computerConfiguration.frontPanelConfiguration.tilType = TIL311;
                        computerConfiguration.frontPanelConfiguration.type = PANEL_ELFII;
                        parseXml_FrontPanel (*child);
                    }
                    if (child->GetAttribute("type") == "quest")
                    {
                        computerConfiguration.frontPanelConfiguration.tilType = TIL313ITALIC;
                        computerConfiguration.frontPanelConfiguration.type = PANEL_SUPER;
                        parseXml_FrontPanel (*child);
                    }
                    if (child->GetAttribute("type") == "elf2k")
                    {
                        computerConfiguration.frontPanelConfiguration.tilType = TIL311;
                        computerConfiguration.frontPanelConfiguration.type = PANEL_ELF2K;
                        parseXml_FrontPanel (*child);
                    }
                    if (child->GetAttribute("type") == "microtutor")
                    {
                        computerConfiguration.frontPanelConfiguration.tilType = TIL311;
                        computerConfiguration.frontPanelConfiguration.type = PANEL_MICROTUTOR;
                        parseXml_FrontPanel (*child);
                    }
                    if (child->GetAttribute("type")  == "microtutor2" || child->GetAttribute("type")  == "microtutorII")
                    {
                        computerConfiguration.frontPanelConfiguration.tilType = TIL313ITALIC;
                        computerConfiguration.frontPanelConfiguration.type = PANEL_MICROTUTOR2;
                        parseXml_FrontPanel (*child);
                    }
                }
                else
                {
                    computerConfiguration.frontPanelConfiguration.type = PANEL_XML;
                    computerConfiguration.frontPanelConfiguration.tilType = TILNONE;
                    parseXml_FrontPanel (*child);
                }
            break;
         ...
         /*       if (mode_.gui)
                {
                    if (computerInfo.configuration == "")
                        ...
                        guiName = computerInfo.name + " - " + computerInfo.configuration;
                    guiName.Replace("&","&&");
...
void XmlParser::parseXml_IdeDisk(wxXmlNode &node)
{
    ...
    while (child)
    {
        ...
        {
            case TAG_FILENAME:
                computerConfiguration.ideFileConfiguration.fileName = child->GetNodeContent();
            break;

            case TAG_DIRNAME:
                computerConfiguration.ideFileConfiguration.directory = dataDir_ + child->GetNodeContent();
                if (computerConfiguration.ideFileConfiguration.directory.Right(1) != pathSeparator_)
                    computerConfiguration.ideFileConfiguration.directory += pathSeparator_;
            break;
    ...
    while (child)
    {
        ...
        switch (tagTypeInt)
        {
            ...
            case TAG_XSCALE:
                ...
                computerConfiguration.coinConfiguration.xScale = getDouble(child->GetNodeContent(), childName, -1, "");
                screenInfo.borderX[VIDEOCOIN] = (int)screenInfo.borderX[VIDEOCOIN] / computerConfiguration.coinConfiguration.xScale;
            break;
    ...
    while (child)
    {
        ...
        switch (tagTypeInt)
        {
            ...
            case TAG_XSCALE:
                ...
                computerConfiguration.cdp1861Configuration.xScale = getDouble(child->GetNodeContent(), childName, -1, "");
                screenInfo.borderX[VIDEOXMLPIXIE] = (int)screenInfo.borderX[VIDEOXMLPIXIE] / computerConfiguration.cdp1861Configuration.xScale;
            break;
    ...
    while (child)
    {
        ...
        switch (tagTypeInt)
        {
            ...
            case TAG_XSCALE:
                ...
                computerConfiguration.cdp1864Configuration.xScale = getDouble(child->GetNodeContent(), childName, -1, "");
                screenInfo.borderX[VIDEOXML1864] = (int)screenInfo.borderX[VIDEOXML1864] / computerConfiguration.cdp1864Configuration.xScale;
            break;
    ...
    while (child)
    {
        ...
        switch (tagTypeInt)
        {
            ...
            case TAG_XSCALE:
                ...
                computerConfiguration.studio4VideoConfiguration.xScale = getDouble(child->GetNodeContent(), childName, -1, "");
                screenInfo.borderX[VIDEOSTUDIOIV] = (int)screenInfo.borderX[VIDEOSTUDIOIV] / computerConfiguration.studio4VideoConfiguration.xScale;
            break;
    ...
    while (child)
    {
        ...
        switch (tagTypeInt)
        {
            ...
            case TAG_XSCALE: 
                ...
                computerConfiguration.vip2KVideoConfiguration.xScale = getDouble(child->GetNodeContent(), childName, -1, "");
                screenInfo.borderX[VIDEOVIP2K] = (int)screenInfo.borderX[VIDEOVIP2K] / computerConfiguration.vip2KVideoConfiguration.xScale;
            break;
    ...
    while (child)
    {
        ...
        switch (tagTypeInt)
        {
            ...
            case TAG_XSCALE:
                ...
                computerConfiguration.fredVideoConfiguration.xScale = getDouble(child->GetNodeContent(), childName, -1, "");
                screenInfo.borderX[VIDEOFRED] = (int)screenInfo.borderX[VIDEOFRED] / computerConfiguration.fredVideoConfiguration.xScale;
            break;
    ...
    while (child)
    {
        ...
        switch (tagTypeInt)
        {
            case TAG_PAGE:
                ...
                computerConfiguration.vis1870Configuration.pageMemSize =  (int)parseXml_Number(*child);
            break;
                                
            case TAG_CHAR:
                if (child->GetAttribute("type") == "rom")
                    computerConfiguration.vis1870Configuration.charMemIsRom = true;
                computerConfiguration.vis1870Configuration.charMemSize =  (int)parseXml_Number(*child);
            break;
            ...
            case TAG_CLOCK:
                computerConfiguration.vis1870Configuration.videoClock = getDouble(child->GetNodeContent(), childName, 500, "500");
            break;
...
void XmlParser::parseXml_ElfKeypad(wxXmlNode &node)
{
    ...
    if (computerConfiguration.frontPanelConfiguration.type == PANEL_ELF2K)
    {
        computerConfiguration.useHex = true;
        computerConfiguration.useSwitch = false;
    }
...
void XmlParser::parseXml_FrontPanel(wxXmlNode &node)
{
    computerConfiguration.hexDisplayConfiguration.defined = true;
    ...
    if (computerConfiguration.useHexKeyboard && computerConfiguration.frontPanelConfiguration.type == PANEL_ELF2K)
    {
        computerConfiguration.useHex = true;
        computerConfiguration.useSwitch = false;
    }
    ...
    while (child)
    {
        ...
        switch (tagTypeInt)
        {
            ...
            case TAG_IN:
                if (computerConfiguration.frontPanelConfiguration.type != PANEL_COSMAC && computerConfiguration.frontPanelConfiguration.type != PANEL_ELF2K)
                    computerConfiguration.useHexKeyboard = true;
                if (child->GetAttribute("type") == "multitil")
            ...
            case TAG_TIL:
                computerConfiguration.frontPanelConfiguration.tilType = TIL311;
                if (child->GetNodeContent() == "313")
                    computerConfiguration.frontPanelConfiguration.tilType = TIL313;
                if (child->GetAttribute("font") == "italic")
                    computerConfiguration.frontPanelConfiguration.tilType = TIL313ITALIC;
            break;
                
            case TAG_LED:
                if (computerConfiguration.frontPanelConfiguration.type == PANEL_COSMAC)
                {
                    computerConfiguration.ledDisplayConfiguration.defined = true;
                    computerConfiguration.ledDisplayConfiguration.ioGroupVector.clear();
                    computerConfiguration.ledDisplayConfiguration.output = parseXml_IoPort(*child, LED_MODULE_OUT);
                }
            break;

            case TAG_SWITCH:
                if (computerConfiguration.frontPanelConfiguration.type == PANEL_ELF2K)
                    computerConfiguration.useSwitch = true;
                computerConfiguration.useHex = false;
            break;

            case TAG_ITEM:
                parseXml_FrontPanelItem (*child);
            break;
            ...
            case TAG_SIZE:
                ...
                computerConfiguration.frontPanelConfiguration.size = wxSize(xSize, ySize);
            break;
...
void XmlParser::parseXml_FrontPanelItem(wxXmlNode &node)
{
    computerConfiguration.frontPanelConfiguration.flexPanel = true;
    ...
    computerConfiguration.guiItemConfiguration.resize(guiItemConfigNumber_+1);
    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = SWITCH_BUTTON_VERTICAL;
    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value = -1;
    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].rangeLow = -1;
    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].rangeHigh = -1;
    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].function = -1;
    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].actOnRelease = false;
    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].actOnPress = false;
    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].label = "";
    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].position = wxPoint(0,0);
    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].textSize = 12;
    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].initup = BUTTON_DOWN;
    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].dirName = computerConfiguration.mainDir_;
    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].useImageDir = false;
    ...
    while (child)
    {
        ...
        switch (tagTypeInt)
        {
            case TAG_TYPE:
                ...
                {
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].actOnRelease = true;
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = SWITCH_BUTTON_VERTICAL;
                    if (child->HasAttribute("direction"))
                    {
                        if (child->GetAttribute("direction") == "horizontal")
                            computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = SWITCH_BUTTON_HORIZONTAL;
                    }
                }
                ...
                {
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].actOnRelease = true;
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = DIP_SWITCH_BUTTON;
                }

                if (child->GetNodeContent() == "rot")
                {
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].actOnRelease = true;
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = ROT_SWITCH_BUTTON;
                }

                if (child->GetNodeContent() == "spin_adi")
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = ADI_SPINCTRL;

                if (child->GetNodeContent() == "spin_adi_volt")
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = ADI_VOLT_SPINCTRL;

                if (child->GetNodeContent() == "spin_ads")
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = ADS_SPINCTRL;

                if (child->GetNodeContent() == "spin_ads_volt")
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = ADS_VOLT_SPINCTRL;

                if (child->GetNodeContent() == "push")
                {
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].actOnPress = true;
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = PUSH_BUTTON;
                    if (child->HasAttribute("size"))
                    {
                        if (child->GetAttribute("size") == "small")
                            computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = PUSH_BUTTON_SMALL;
                    }
                    ...
                    if (child->HasAttribute("form"))
                    {
                        ...
                        {
                            if (computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type == PUSH_BUTTON_SMALL)
                                computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = PUSH_BUTTON_RECTANGLE_SMALL;
                            else
                                computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = PUSH_BUTTON_RECTANGLE;
                        }
                        if (child->GetAttribute("form") == "round")
                        {
                            computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = PUSH_BUTTON_ROUND_BLACK;
                        }
                    }
                    ...
                    if (child->HasAttribute("color"))
                    {
                        ...
                        {
                            if(computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type == PUSH_BUTTON)
                                computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = PUSH_BUTTON_RED;
                            if(computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type == PUSH_BUTTON_ROUND_BLACK)
                                computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = PUSH_BUTTON_ROUND_RED;
                        }
                    }
                }
                ...
                {
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].color = GUI_COL_BLACK;
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = PANEL_TEXT;
                    if (child->HasAttribute("color"))
                    {
                        if (child->GetAttribute("color") == "grey")
                        {
                            computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].color = GUI_COL_GREY;
                        }
                    }
                }
                ...
                {
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = PANEL_PNG;
                }
                
                if (child->GetNodeContent() == "led")
                {
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = LED_SMALL_RED;
                    if (child->GetAttribute("form") == "realred")
                        computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = LED_REAL_RED;
                    if (child->GetAttribute("form") == "realgreen")
                        computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = LED_REAL_GREEN;
                    if (child->GetAttribute("form") == "realorange")
                        computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = LED_REAL_ORANGE;
                    if (child->GetAttribute("form") == "large")
                        computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = LED_LARGE_RED;
                    if (child->GetAttribute("form") == "largegreen")
                        computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = LED_LARGE_GREEN;
                    if (child->GetAttribute("form") == "disable")
                        computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = LED_SMALL_RED_DISABLE;
                }

                if (child->GetNodeContent() == "til")
                {
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = TIL_311;
                    if (child->GetAttribute("form") == "313")
                        computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = TIL_313;
                    if (child->GetAttribute("font") == "italic")
                        computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].type = TIL_313_ITALIC;
                }
            break;

            case TAG_VALUE:
                computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value = (int)parseXml_Number(*child);
                if (computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].function >= BUTTON_FUNC_IN && computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].function < BUTTON_FUNC_CARD)
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value = computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].function;
                
                if (child->HasAttribute("address"))
                    addressLine = (int)parseXml_Number(*child, "address");

                switch (computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].function)
                {
                    case BUTTON_FUNC_DIPMP:
                        computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value += 0x50;
                    break;

                    case BUTTON_FUNC_EMS:
                        computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value += 0x60;
                    break;

                    case BUTTON_FUNC_TMC_AD:
                        computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value += 0x70;
                    break;

                    case BUTTON_FUNC_BIT:
                        computerConfiguration.frontPanelConfiguration.dataSwitches = true;
                        computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value += 0x80;
                    break;

                    case BUTTON_FUNC_CARD:
                        computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value += 0x100;
                    break;

                    case LED_FUNC_BIT:
                        if (computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value >= 8)
                            computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value = 0;
                        if (computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value != -1)
                            computerConfiguration.ledDisplayConfiguration.bit[computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value] = true;
                    break;

                    case LED_FUNC_ADDRESS:
                        if (computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value >= MAX_BIT_LEDS - 8)
                            computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value = 0;
                        if (computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value != -1)
                            computerConfiguration.ledDisplayConfiguration.bit[computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value+8] = true;
                    break;

                    case LED_FUNC_CPUSTATE:
                        if (computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value >= MAX_CPU_STATE_LEDS)
                            computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value = 0;
                        if (computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value != -1)
                            computerConfiguration.ledDisplayConfiguration.cpuStatus[computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value] = true;
                    break;
                        
                    case TIL_DATA:
                        if (computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value >= MAX_DATA_TIL)
                            computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value = 0;
                        if (computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value != -1)
                            computerConfiguration.ledDisplayConfiguration.datatil[computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value] = true;
                    break;

                    case TIL_ADDRESS:
                        if (computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value >= MAX_ADDRESS_TIL)
                            computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value = 0;
                        if (computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value != -1)
                            computerConfiguration.ledDisplayConfiguration.addresstil[computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value] = true;
                    break;

                    case TIL_MULTI:
                        if (computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value >= MAX_MULTI_TIL)
                            computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value = 0;
                        if (computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value != -1)
                            computerConfiguration.multiSegDisplayConfiguration.multitil[computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value] = true;
                        if (addressLine != -1)
                            computerConfiguration.multiSegDisplayConfiguration.multitilAddressLine[computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value] = addressLine;
                    break;
                }
            break;
            ...
            case TAG_RANGE:
                ...
                {
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].rangeLow = rangeHigh;
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].rangeHigh = rangeLow;
                }
                else
                {
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].rangeLow = rangeLow;
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].rangeHigh = rangeHigh;
                }
            break;
            ...
            case TAG_FUNCTION:
                ...
                while (itemFunctionList[itemFunctionId] != "undefined")
                {
                    ...
                    {
                        computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].function = itemFunctionValueList[itemFunctionId];
                        if (itemFunctionId >= BUTTON_FUNC_IN && itemFunctionId < BUTTON_FUNC_CARD)
                            computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value = itemFunctionId+20;
                        ...
                        switch (itemFunctionId)
                        {
                            ...
                            case BUTTON_FUNC_POWER:
                                computerConfiguration.frontPanelConfiguration.powerButtonDefined = true;
                            break;

                            case BUTTON_FUNC_WAIT:
                                computerConfiguration.frontPanelConfiguration.waitButtonDefined = true;
                            break;
                            ...
                            case LED_FUNC_Q:
                                computerConfiguration.ledDisplayConfiguration.q = true;
                            break;

                            case LED_FUNC_ERROR:
                                computerConfiguration.ledDisplayConfiguration.error = true;
                            break;

                            case LED_FUNC_RUN:
                                computerConfiguration.ledDisplayConfiguration.run = true;
                            break;

                            case LED_FUNC_LOAD:
                                computerConfiguration.ledDisplayConfiguration.load = true;
                            break;

                            case LED_FUNC_RESET:
                                computerConfiguration.ledDisplayConfiguration.reset = true;
                            break;

                            case LED_FUNC_PAUSE:
                                computerConfiguration.ledDisplayConfiguration.pause = true;
                            break;

                            case LED_FUNC_READY:
                                computerConfiguration.ledDisplayConfiguration.ready = true;
                            break;

                            case LED_FUNC_STOP:
                                computerConfiguration.ledDisplayConfiguration.stop = true;
                            break;

                            case LED_FUNC_POWER:
                                computerConfiguration.ledDisplayConfiguration.power = true;
                            break;

                            case LED_FUNC_NIBBLE:
                                if (computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value != -1)
                                    computerConfiguration.ledDisplayConfiguration.nibble[computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value&1] = true;
                            break;
                            ...
                            case TIL_MULTI:
                                if (computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value != -1)
                                    computerConfiguration.multiSegDisplayConfiguration.multitil[computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value] = true;
                                if (computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value > computerConfiguration.multiSegDisplayConfiguration.multiTilNumber)
                                {
                                    computerConfiguration.multiSegDisplayConfiguration.multiTilNumber = computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].value;
                                    if (addressLine != -1)
            ...
            case TAG_LABEL:
                computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].label = child->GetNodeContent();
                if (child->HasAttribute("size"))
#if defined (__WXMAC__)
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].textSize = (int)parseXml_Number(*child, "size")+4;
#else
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].textSize = (int)parseXml_Number(*child, "size");
#endif
            break;
                
            case TAG_INIT:
                if (child->GetNodeContent() == "up")
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].initup = BUTTON_UP;
            break;
            ...
            case TAG_POS:
                ...
                xpos = (int)getNextHexDec(&position);
                ypos = (int)getNextHexDec(&position);
                computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].position = wxPoint(xpos, ypos);
            break;
            ...
            case TAG_SIZE:
                ...
                computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].size = wxSize(xSize, ySize);
            break;
            ...
            case TAG_FILENAME:
                if (child->GetAttribute("dirname") == "images")
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].useImageDir = true;
                computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].fileName = child->GetNodeContent();
            break;

            case TAG_DIRNAME:
                computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].dirName = dataDir_ + child->GetNodeContent();
                if (computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].dirName != pathSeparator_)
                    computerConfiguration.guiItemConfiguration[guiItemConfigNumber_].dirName += pathSeparator_;
            break;
...
void XmlParser::parseXml_HwCassette(wxXmlNode &node)
{
    ...
    while (child)
    {
        ...
        switch (tagTypeInt)
        {
            ...
            case TAG_IN:
                if (child->HasAttribute("mode"))
                    computerConfiguration.hwTapeConfiguration.mode = (int)parseXml_Number(*child, "mode");
...
IoPort XmlParser::parseXml_IoPort(wxXmlNode &node, int ioDefinition, Byte defaultMask)
{
    ...
    if (node.HasAttribute("addressmask"))
        ioPort.addressMask = (Byte)parseXml_Number(node, "addressmask");
